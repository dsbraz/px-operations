...
2. **Aggregates crossing into the API.** Projects returns the aggregate and the controller maps it (`ProjectsController.cs:68`); Milestones and Nps use Views (`MilestoneView`, `NpsProjectView`) — the Views are the pattern to follow. ProjectHealth sits in between: its commands hand the aggregate back to the controller and `ProjectHealthMappings` parses a Domain enum (`ParseRagStatus`), but every response is mapped from `ProjectHealthView` — the controller never maps the aggregate, so a new action there has no excuse to. Note `DependencyRulesTests` only asserts on `.csproj` references, so it does not catch the `using PxOperations.Domain.*` in `ProjectsController.cs`.
...
6. **A verb in an Nps route.** `GET /api/nps/responses/export` puts an action in the path. Renaming it breaks the published contract, so it stays until the whole surface is versioned — see *Out of scope* reasoning in the module's PRD. Do not add a second one.
7. **A unit test in the integration project.** `SubmitNpsPublicResponseUseCaseTests` uses fakes and needs no container, but there is no Application unit-test project to hold it. Creating one for a single file is not worth it today.
