...
4. **Duplicated error mapping.** `catch (BusinessRuleValidationException) → BadRequest(ProblemDetails)` is repeated across four controllers. `AddProblemDetails()` and `UseExceptionHandler()` are already wired in `Program.cs`; the single handler needs the exception split from the REST gate above.
5. **Verb in a path.** `PATCH /api/nps/dispatches/{id}/close`.
