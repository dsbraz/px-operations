...
public partial class MilestonesToolbar : ComponentBase
{
    ...
    [Parameter, EditorRequired] public IReadOnlyList<string> ProjectTypes { get; set; } = [];
    ...
    [Parameter] public string FilterProjectType { get; set; } = string.Empty;
    ...
    private Task OnProjectTypeChanged(ChangeEventArgs args) => OnFilterProjectTypeChanged.InvokeAsync(args.Value?.ToString() ?? string.Empty);
