...
public partial class ProjectHealthDashboardPage : ComponentBase
{
    ...
    private async Task LoadDataAsync()
    {
        ...
        try
        {
            ...
            var projectType = string.IsNullOrEmpty(filterProjectType) ? null : filterProjectType;
            var week = string.IsNullOrEmpty(filterWeek) ? null : filterWeek;

            // Summary represents the active carteira for the period: scoped by DC, project type, and week.
            // Busca/Nota narrow the lists below, not the headline truths.
            summary = await ProjectHealthClient.GetSummaryAsync(null, dc, projectType, null, week, null, null, default);
            availableWeeks = summary?.WeeklyEvolution?.Select(w => w.Week).Reverse().ToList() ?? [];
            await FetchEntriesAsync(dc, projectType, week);
        }
        catch (Exception)
        {
    ...
    private async Task FetchEntriesAsync(string? dc, string? projectType, string? week)
    {
        int? minScore = filterScore switch { "hi" => 7, "md" => 4, "lo" => 0, _ => null };
        int? maxScore = filterScore switch { "hi" => 10, "md" => 6, "lo" => 3, _ => null };
        entries = (await ProjectHealthClient.List2Async(null, dc, projectType, null, week, minScore, maxScore, default)).ToList();
    }
    ...
    private async Task OnFilterProjectTypeChanged(string value) { filterProjectType = value; filterWeek = ""; await LoadDataAsync(); }
