...
public partial class HealthFormPage : ComponentBase
{
    ...
    {
        allProjects = (await ProjectsClient.ListAsync(null, null, null, null, null, default)).ToList();

        if (Id.HasValue)
        {
            var entry = await HealthChecksClient.GetById3Async(Id.Value, default);
    ...
    private async Task HandleSubmit()
    {
        ...
        try
        {
            if (Id.HasValue)
            {
                await HealthChecksClient.Update3Async(Id.Value, new UpdateHealthCheckRequest
                {
            ...
            {
                await HealthChecksClient.Create3Async(new CreateHealthCheckRequest
                {
                    ProjectId = formProjectId,
                    SubProject = string.IsNullOrWhiteSpace(formSubProject) ? null : formSubProject,
                    Week = week,
