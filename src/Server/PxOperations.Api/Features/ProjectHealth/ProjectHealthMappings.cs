using PxOperations.Api.Features.ProjectHealth.Contracts;
using PxOperations.Application.Features.ProjectHealth;
using PxOperations.Domain.ProjectHealth;
using PxOperations.Domain.Projects;
using ProjectMappings = PxOperations.Api.Features.Projects.ProjectMappings;

namespace PxOperations.Api.Features.ProjectHealth;

public static class ProjectHealthMappings
{
    public static ProjectHealthResponse ToResponse(ProjectHealthView view)
    {
        return new ProjectHealthResponse(
            view.Id,
            view.ProjectId,
            view.ProjectName,
            view.ProjectClient,
            view.ProjectDc,
            view.ProjectDeliveryManager,
            view.SubProject,
            view.Week,
            view.ReporterEmail,
            view.PracticesCount,
            view.Scope,
            view.Schedule,
            view.Quality,
            view.Satisfaction,
            view.Score,
            view.ExpansionOpportunity,
            view.ExpansionComment,
            view.ActionPlanNeeded,
            view.ActionPlanComment,
            view.Highlights);
    }

    public static ProjectHealthSummaryResponse ToSummaryResponse(ProjectHealthSummary summary)
    {
        return new ProjectHealthSummaryResponse(
            summary.TotalEntries,
            summary.TotalProjects,
            summary.AverageScore,
            summary.OverallAverageScore,
            summary.AverageScope,
            summary.AverageSchedule,
            summary.AverageQuality,
            summary.AverageSatisfaction,
            summary.CriticalCount,
            summary.OverallCriticalCount,
            summary.AttentionCount,
            summary.HealthyCount,
            summary.NoResponseCount,
            summary.OverallNoResponseCount,
            summary.WithExpansionCount,
            summary.WithActionPlanCount,
            summary.WeeklyEvolution.Select(w =>
                new WeeklyScorePointResponse(w.Week, w.AverageScore, w.EntryCount)).ToList());
    }

    public static RagStatus ParseRagStatus(string value)
    {
        return value.ToLowerInvariant() switch
        {
            "green" or "verde" => RagStatus.Green,
            "yellow" or "amarelo" => RagStatus.Yellow,
            "red" or "vermelho" => RagStatus.Red,
            _ => throw new ArgumentException($"Invalid RAG status: {value}")
        };
    }

    public static DeliveryCenter? ParseDeliveryCenterOrNull(string? value)
        => value is null ? null : ProjectMappings.ParseDeliveryCenter(value);
}
