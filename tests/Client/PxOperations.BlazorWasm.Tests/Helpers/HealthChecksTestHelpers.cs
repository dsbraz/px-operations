...
{
    internal static HealthCheckResponse MakeHealthCheck(
        int id = 1,
        int projectId = 1,
        ...
        string highlights = "Tudo certo.") => new HealthCheckResponse
        {
            Id = id,
            ProjectId = projectId,
    ...
    internal static string HealthCheckJson(HealthCheckResponse h)
    {
    ...
    internal static string HealthChecksJson(params HealthCheckResponse[] items)
        => $"[{string.Join(",", items.Select(HealthCheckJson))}]";
