...
public sealed class HealthDashboardPageTests : TestContext
{
    [Fact]
    public void Page_should_show_loading_while_api_is_pending()
    {
        ...
        var cut = RenderComponent<HealthDashboardPage>();
    ...
    public void Page_should_render_stats_bar_with_summary_data()
    {
        ...
        handler.AddResponse(HttpMethod.Get, HealthChecksTestHelpers.SummaryJson(
            totalEntries: 5, totalProjects: 3, avgScore: 7.5, criticalCount: 1, noResponseCount: 2
        ), HttpStatusCode.OK);
        handler.AddResponse(HttpMethod.Get, HealthChecksTestHelpers.HealthChecksJson(
            HealthChecksTestHelpers.MakeHealthCheck(1, score: 10, highlights: "Tudo ótimo")
        ), HttpStatusCode.OK);
    ...
    {
        var handler = new ProjectsTestHelpers.MultiStubHttpMessageHandler();
        handler.AddResponse(HttpMethod.Get, HealthChecksTestHelpers.SummaryJson(totalProjects: 1), HttpStatusCode.OK);
        handler.AddResponse(HttpMethod.Get, HealthChecksTestHelpers.HealthChecksJson(
            HealthChecksTestHelpers.MakeHealthCheck(1, highlights: "Destaque da semana")
        ), HttpStatusCode.OK);
    ...
    {
        var handler = new ProjectsTestHelpers.MultiStubHttpMessageHandler();
        handler.AddResponse(HttpMethod.Get, HealthChecksTestHelpers.SummaryJson(), HttpStatusCode.OK);
        ...
        cut.WaitForAssertion(() =>
        {
            Assert.Contains("/health/new", cut.Markup);
